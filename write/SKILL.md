...
3. **Language detected** from the text being edited, not the user's command:
   ...
   - **Mixed language unidentifiable** → ask user "这段的主要语言是中文还是英文？" before proceeding

Read the loaded reference file. Then edit. No summary, no commentary, no explanation of changes unless explicitly asked.

## Post-edit length validation（刘同学规则）

当文本属于微头条 或 公众号长文 场景时，编辑完成后必须验证字数：
- 微头条：500-1500 字
- 公众号长文：3000-8000 字
- 超出范围 → 自动压缩或扩充至合格区间再交付，不提示用户
...
- **Length compliance.** For 刘同学 微头条/公众号长文场景, post-edit word count must be in range (微头条 500-1500, 公众号 3000-8000). Adjust silently — no user prompt.

## 朱雀AI检测过检经验（刘同学实测）
 
以下规律经刘同学长达19轮迭代改写实测验证，适用于公众号长文和微头条的朱雀检测过检场景。

### 核心规律

1. **字数必须>500字**才能过检，太短必然被判AI
2. **不要每句话都有因果关系**，真人写东西有跳跃，不需要句句衔接
3. **不用结构化引导**：禁用"先说结论""我建议""给大家总结一下""接下来我们聊聊"
4. **家庭打断是安全区**：配偶对话、子女互动等日常场景可以自然打断叙事节奏，让检测器感知到"活人感"
5. **内心矛盾永远比"想通了"更真**：表达纠结、犹豫、反复横跳的情绪，不要强行升华或给出明确答案
6. **结尾抛问不下结论**：用提问收尾，不要总结陈词、不要升华拔高
7. **段落打散**：每段1-3句，对话单独成段，不要出现超过4句的段落墙

### 高概率AI特征（必须清除）

| 特征 | 说明 |
|------|------|
| 太"圆"的结构 | 每件事都有头有尾，衔接太顺，像精心编排过 |
| 因果闭环 | 每个分句都解释前一句，"因为所以"全写满了 |
| 结论先行 | "先说结论""总的来说""归根结底" |
| 升华结尾 | "通过这件事，我们明白了……" |
| 刻意对比 | "Cursor够用但不够好，Claude好但贵"这种工整对比 |
| 数字列表体 | "第一""第二""第三"或带编号的罗列 |
| 工具推荐列表 | 列出多个工具做横向对比 |
| 完美结尾 | 故事完整、情绪解决、干净收束 → 这恰恰是最AI的 |

### 通过验证的"真人感"写法

- 叙事中途插入无关细节（同事搭话、老婆问话、孩子跑过来）
- 用具体数字代替模糊描述（"八美金" > "花了不少钱"）
- 情绪不说完，留白（"说不上来什么感觉"）
- 用短句打断长叙述（独立成段的一句话）
- 结尾抛给读者，不下结论，不拔高
...
| 文件 | 主题 | 说明 |
|------|------|------|
| `references/sample_claude_code_cost.md` | Claude Code token费用吐槽 | 经25轮迭代，重点参考"内心矛盾"+ "家庭打断"写法 |
| `references/sample_opencut.md` | OpenCut开源剪辑工具推荐 | 经7轮迭代，重点参考"去编号分节"+ "承认不足"写法 |
